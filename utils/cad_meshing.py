...
def create_cad_mesh(generator, primitives, connection_t, filename, file_type='off', p_dim=1024, \
		...
		if unused_convex[i]==0:
			...
			for j in range(p_dim):
				if connection_t[0,j,i]>0:
					...
					mesh_new = trimesh.Trimesh(vertices=vertices, faces=triangles)
					if not mesh_new.is_volume:
						repair.fix_normals(mesh_new)  # 修复法线方向
						repair.fill_holes(mesh_new)   # 填补孔洞
					if mesh_new.is_watertight and mesh_new.is_volume:
					    print("mesh_new has been successfully repaired.")
					else:
						print("Failed to repair the mesh_new.")
						print("Press Enter to continue...")
						input()
						break

					if convex_mesh is None:
						convex_mesh = mesh_new
					else:
						convex_mesh = convex_mesh.intersection(mesh_new)
					...
					if inter_results_flag:
						...
						file_path = f"{primi_dir}/{i}_{j}.{file_type}"
						mesh.export(file_path)
						print(original_primitives[:,:,j:j+1])
						primitive_slice = original_primitives[:, :, j:j+1].squeeze().T# 提取原始数据并转置以匹配所需的形状 (7, 1)						    
						if primitive_slice.ndim == 1:# 确保 primitive_slice 形状为 (1, 7)
							primitive_slice = primitive_slice.reshape(1, -1)
						print(primitive_slice)
						i_tensor = np.array([[float(i)]])# 将 i 转换为浮点数并创建一个新的张量
						print(i_tensor)
						combined_vector = np.hstack((i_tensor, primitive_slice))# 水平拼接 j 和原始数据，形成 (1, 8) 形状的向量
						csg_tree.append(combined_vector)# 添加到 csg_tree 列表
						print(csg_tree)
					
			if inter_results_flag:		
						file_path = f"{convex_dir}/{i}.{file_type}"
						convex_mesh.export(file_path)
			
			if i < c_dim_half:
				if concave_mesh_left is None:
					concave_mesh_left = convex_mesh
				else:
					concave_mesh_left = concave_mesh_left.union(convex_mesh)
			else:
				if concave_mesh_right is None:
					concave_mesh_right = convex_mesh
				else:
					concave_mesh_right = concave_mesh_left.union(convex_mesh)
	csg_tree_array = np.vstack(csg_tree)#用于将一个列表中的多个一维或二维数组垂直堆叠（即按行堆叠）成一个新的二维数组
	csg_tree_array.astype('float32').tofile('./yyh_click_it/csg_tree.bin')# 将数组保存为二进制文件
	if concave_mesh_left is None:
		if concave_mesh_right is None:
			print('empty mesh!!!')
			exit()
			...
			if inter_results_flag:
				file_path = f"{concave_dir}/right.{file_type}"
				concave_mesh_right.export(file_path)
	else:
		if concave_mesh_right is None:
			output_mesh = concave_mesh_left
			if inter_results_flag:
				file_path = f"{concave_dir}/left.{file_type}"
				concave_mesh_left.export(file_path)
		else:
			output_mesh = concave_mesh_left.difference(concave_mesh_right)	
			if inter_results_flag:
				file_path = f"{concave_dir}/left.{file_type}"
				concave_mesh_left.export(file_path)
				file_path = f"{concave_dir}/right.{file_type}"
	...
	if final_result_flag:
		file_name = os.path.splitext(os.path.basename(ply_filename))[0]
		output_path = os.path.join(os.path.dirname(ply_filename), f"{file_name}.{file_type}")
